...
<script type="text/javascript">
	function exportList(){
		$("#pageForm").attr("action","${ctx}/logs/operationLog!exportList.do");
		$("#pageForm").submit();
		$("#pageForm").attr("action","${ctx}/logs/operationLog!findPage.do");
	}
</script>
</head>
<body class="main_bg">
	<div class="win-header" id="a">
		<span class="win-header-title">日志管理 &gt; 客户端操作日志 </span>
	</div>
	<div class="win-bodyer">
		<form id="pageForm" name="pageForm" action="${ctx}/logs/operationLog!findPage.do" method="post">
		<div class="win-advsearch">
			<table width="100%">
				<tr>
					...
					<td>用户名：
						<input type="text" id="pageObject.params.userName" name="pageObject.params.userName" value="${pageObject.params.userName }"/>
					</td>					
					<td>终端ip：
		...
		<div id="pageDiv" name="pageDiv" class="pageDiv">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<thead>
					<tr>
						<th width="15%"><div>用户编码</div></th>
						<th width="15%">用户名</th>
						<th width="10%"><div>登录IP</div></th>
						<th width="10%">操作类型</th>
				...
				<tbody>
				<c:forEach var="operationLog" items="${pageObject.resultList}">
			    	<tr>
				    	<td>${operationLog.userId}</td>
				    	<td>${operationLog.userName}</td>
				    	<td>${operationLog.terminalIp}</td>
						<td> <c:forEach var="type" items="${types}">
								<c:if test="${operationLog.operation eq type.value}">${type.label}</c:if>
							 </c:forEach>
						</td>
						<td>${operationLog.description}</td>
						<td>${operationLog.time}</td>
						<td>${operationLog.objectTime}</td>
			    	</tr>
				</c:forEach>
				</tbody>
				<tfoot>
