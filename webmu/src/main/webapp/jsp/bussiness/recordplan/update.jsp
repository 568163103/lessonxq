	...
	<script type="text/javascript">
		$(function() {
			...
			$("#selectable li").each(function(i,e){
				if($("#recordPlan\\.timespan").val().charAt(i) == "1"){
					$(e).addClass("ui-selected").attr("value",'1');
				}
			});
		});
		function edit() {
...
<body class="main_bg">
	...
	<div class="win-bodyer" align="center">
		<form id="pageForm" name="pageForm" action="${ctx}/bussiness/recordplan!update.do" method="post" onsubmit="getTimeSpan();">
			<input type="hidden" id="oname" name="oname" value="${name}">
			...
			<table width="100%" class="inputTable" border="0">
				<tr>
					...
					<td width="30%">
						<input type="text" value="${recordPlan.name}" class="validate[required,ajax[checkUnique]]" name="recordPlan.name" id="recordPlan_name_${recordPlan.name}" readonly="readonly" />
					</td>
