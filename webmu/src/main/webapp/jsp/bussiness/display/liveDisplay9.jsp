...
<head>
	...
	<script type="text/javascript">
		var currChannelId = null,currChannelName = null,$playerIframe = null,currNode = null,currIndex = -1,maxPlayTime = 0;
		function handleCurrIndex() {
			if(currIndex == 8){
				currIndex = -1;
			}
			currIndex++;
			$playerIframe.each(function (i,e) {
				if(e.contentWindow.playerInstance){
					if ("buffering,playing".indexOf(e.contentWindow.playerInstance.getState())<0){
						currIndex = i;
						return false;
					}
				} else {
		...
		function openPlayInIframe(channelId,channelName) {
			...
			if($playerIframe[currIndex].contentWindow.playerInstance){$playerIframe[currIndex].contentWindow.playerInstance.stop();}
			$($playerIframe[currIndex]).loading("加载中，请稍后");
			$($playerIframe[currIndex]).attr("src", "${ctx}/jsp/bussiness/display/player.jsp?mduIp=${mduIp}&mduPort=${mduPort}&channelId="+currChannelId+"&channelName="+currChannelName+"&noTitle=true");
		}
		...
		function playOrStop(obj){
			...
			$playerIframe[currIndex].contentWindow.playerInstance.stop();
		...
		$(function(){
			...
			$playerIframe = $("[name=playerIframe]");
...
<body class="main_bg">
<div class="win-header" id="a" style="min-width: 1660px">
	<span class="win-header-title"><c:if test="${param.u eq '1'}">地图模式</c:if><c:if test="${param.u ne '1'}">视频浏览</c:if> &gt; 实时视频</span>
	<span class="win-header-title" style="cursor: pointer;float:right" onclick="javascript:window.location.href='${ctx}/bussiness/display!beforeLiveDisplay.do?cc=9'">九分屏&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
	<span class="win-header-title" style="cursor: pointer;float:right" onclick="javascript:window.location.href='${ctx}/bussiness/display!beforeLiveDisplay.do'">四分屏&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
	<span class="win-header-title" style="cursor: pointer;float:right" onclick="javascript:window.location.href='${ctx}/bussiness/display!beforeLiveDisplay.do?cc=1'">一分屏&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
</div>
<div class="win-bodyer" style="min-width: 1340px">
		<table width="99%" class="inputTable" border="0">
			<tr>
				<td id="leftTree" align="center" style="width:355px;height:98%;" rowspan="3">
				...
				<td style="text-align: center;vertical-align: middle;" rowspan="1">
					<iframe name="playerIframe" class="playerIframe" frameBorder="0" scrolling="auto" allowtransparency=yes src="${ctx }/jsp/bussiness/display/player.jsp"></iframe>
				</td>
				<td style="text-align: center;vertical-align: middle;" rowspan="1">
					<iframe name="playerIframe" class="playerIframe" frameBorder="0" scrolling="auto" allowtransparency=yes src="${ctx }/jsp/bussiness/display/player.jsp"></iframe>
				</td>
