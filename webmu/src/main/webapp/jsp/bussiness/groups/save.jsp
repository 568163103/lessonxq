...
<body class="main_bg">
	<div class="win-header" id="a">
		<span class="win-header-title">业务管理 &gt; 资源组管理 &gt; 添加资源组</span>
	</div>
	<div class="win-bodyer">
	<form id="pageForm" name="pageForm" action="${ctx}/bussiness/groups!save.do" method="post" onsubmit="getRids()">
		<s:token/>
					...
					<fieldset title="资源组" style="width: 95%;height: 790px;">
						...
						<table class="inputTable" style="width: 100%" border="0">
							<tr>
								...
								<td width="20%">
									<select class="validate[required]" name="groups.position" id="position">
										
										<c:forEach var="type" items="${positions}">
											<option value="${type.key}" <c:if test="${groups.position eq type.key}">selected</c:if>>${type.value}</option>
										</c:forEach>
									</select>
								</td>
							</tr>
							...
							<tr>
								...
								<td style="text-align:left;"><input type="text" value="${groups.name}" class="validate[required,ajax[checkUnique]]" name="groups.name" id="groups_name" /></td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:left;">&nbsp;&nbsp;备注(最多256字符)：</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;width: 98%;vertical-align: top" height="565px">
