...
<title>Insert title here</title>
	...
	function exportList(){
		$("#pageForm").attr("action","${ctx}/dmu/dmuAlarmInfo!exportList2.do");
		$("#pageForm").submit();
		$("#pageForm").attr("action","${ctx}/dmu/dmuAlarmInfo!alarmInfoStatistics.do");
	}
	</script>
</head>
<body class="main_bg">
<div class="win-header" id="a">
	<span class="win-header-title">网管管理 &gt; 告警统计 </span>
</div>
<div class="win-bodyer">
	<form id="pageForm" name="pageForm" action="${ctx}/dmu/dmuAlarmInfo!alarmInfoStatistics.do" method="post">
				...
				<td>告警类型：
						<select class="validate[required]" name="alarmType" id="alarmType">
							...
							<c:forEach var="type" items="${alarmTypes2}">
								<option value="${type.id}" <c:if test="${alarmType eq type.id}">selected</c:if>>${type.name}</option>
							</c:forEach>
						</select>
				</td>
				<td>告警状态：
					<select class="validate[required]" name="status" id="statisticsTime">
						<option value="">请选择</option>
						<option value="1" <c:if test="${status eq '1'}">selected</c:if> >待确认</option>
						<option value="2" <c:if test="${status eq '2'}">selected</c:if> >处理中</option>
						<option value="3" <c:if test="${status eq '3'}">selected</c:if> >待归档</option>
						<option value="4" <c:if test="${status eq '4'}">selected</c:if> >已归档</option>
					</select>
				</td>
				<td>开始时间：
