...
<body class="main_bg">
	<div class="win-header" id="a">
		<span class="win-header-title">网管管理 &gt; 告警级别 &gt; 编辑告警级别</span>
	</div>
	<div class="win-bodyer" align="center">
		<form id="pageForm" name="pageForm" action="${ctx}/dmu/dmuAlarmLevel!update.do" method="post">
			<input type="hidden" id="id" name="id" value="${alarmType.id}">
				<table width="100%" class="inputTable">
					<tr>
						...
						<td width="30%"><input type="text" value="${alarmType.id}" name="alarmType.id" id="id" maxlength="64" disabled="disabled"/></td>
						<td style="text-align: right;">告警类型名称：</td>
						<td width="30%"><input type="text" value="${alarmType.name}" name="alarmType.name" id="name" maxlength="64" disabled="disabled"/></td>
					</tr>
					<tr>
						...
						<td>
							<select name="alarmType.level" id="level" class="validate[required]">
								...
								<c:forEach var="dmuAlarmLevel" items="${dmuAlarmLevels}">
									<option value="${dmuAlarmLevel.value}" <c:if test="${alarmType.level eq dmuAlarmLevel.value}">selected</c:if>>${dmuAlarmLevel.label}</option>
								</c:forEach>
							</select>
						</td>
					</tr>
				...
				<table width="100%" class="inputTable">
					<tr>
						<td colspan="4" style="text-align: center;">
							...
							<a class="a_button" href="${ctx}/dmu/dmuAlarmLevel!findPage.do"><span>返回</span></a>
						</td>
					</tr>
				</table>
		</form>
