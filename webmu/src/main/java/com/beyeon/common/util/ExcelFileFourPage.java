...
public class ExcelFileFourPage {
	...
	private ArrayList<String> fieldName3 = null; //excel标题数据集
	private ArrayList<String> fieldName4 = null; //excel标题数据集

	private ArrayList<ArrayList<String>> fieldData1 = null; //excel数据内容	
	private ArrayList<ArrayList<String>> fieldData2 = null; //excel数据内容	
	private ArrayList<ArrayList<String>> fieldData3 = null; //excel数据内容	
	private ArrayList<ArrayList<String>> fieldData4 = null; //excel数据内容	
	 ...
	 * @param data
	 */
	public ExcelFileFourPage(ArrayList<String> fieldName1, ArrayList<String> fieldName2, ArrayList<String> fieldName3, ArrayList<String> fieldName4,
			ArrayList<ArrayList<String>> fieldData1,ArrayList<ArrayList<String>> fieldData2,ArrayList<ArrayList<String>> fieldData3,ArrayList<ArrayList<String>> fieldData4) {
		...
		this.fieldData3 = fieldData3;
		this.fieldData4 = fieldData4;
	...
	public HSSFWorkbook createWorkbook1() {
		...
		int rows3 = fieldData3.size();//总的记录数
		int sheetNum3 = 0;           //指定sheet的页数

		if (rows3 % SPLIT_COUNT == 0) {
			sheetNum3 = rows3 / SPLIT_COUNT;
		} else {
			sheetNum3 = rows3 / SPLIT_COUNT + 1;
		}
		
		
		int rows4 = fieldData4.size();//总的记录数
		int sheetNum4 = 0;           //指定sheet的页数

		if (rows4 % SPLIT_COUNT == 0) {
			sheetNum4 = rows4 / SPLIT_COUNT;
		} else {
			sheetNum4 = rows4 / SPLIT_COUNT + 1;
		}
		
		int sheetNum = sheetNum1 + sheetNum2 + sheetNum3 + sheetNum4;
		...
		for (int i = 1; i <= sheetNum; i++) {//循环2个sheet的值
			if(i<=sheetNum1){
				...
				for (int k = 0; k < (rows1 < SPLIT_COUNT ? rows1 : SPLIT_COUNT); k++) {
				...
				}else if (i<=sheetNum1 + sheetNum2 + sheetNum3){
					HSSFSheet sheet = workBook.createSheet("Page " + i);//使用workbook对象创建sheet对象
					HSSFRow headRow = sheet.createRow((short) 0); //创建行，0表示第一行（本例是excel的标题）
					for (int j = 0; j < fieldName3.size(); j++) {//循环excel的标题
						HSSFCell cell = headRow.createCell(j);//使用行对象创建列对象，0表示第1列
						/**************对标题添加样式begin********************/
						//设置列的宽度
						sheet.setColumnWidth((short)j, (short)6000);
						...
						if(fieldName1.get(j) != null){
							//将创建好的样式放置到对应的单元格中
							cell.setCellStyle(cellStyle);
							cell.setCellValue((String) fieldName3.get(j));//为标题中的单元格设置值
						}else{
							cell.setCellValue("-");
						}
					}
					//分页处理excel的数据，遍历所有的结果
					for (int k = 0; k < (rows2 < SPLIT_COUNT ? rows2 : SPLIT_COUNT); k++) {
						if (((i - sheetNum1 - sheetNum2 -1) * SPLIT_COUNT + k) >= rows3)//如果数据超出总的记录数的时候，就退出循环
							break;
						HSSFRow row = sheet.createRow((short) (k + 1));//创建1行
						//分页处理，获取每页的结果集，并将数据内容放入excel单元格
						ArrayList<String> rowList = (ArrayList<String>) fieldData3.get((i - sheetNum1 - sheetNum2 -1) * SPLIT_COUNT + k);
						for (int n = 0; n < rowList.size(); n++) {//遍历某一行的结果
							HSSFCell cell = row.createCell(n);//使用行创建列对象
							cell.setCellValue(HSSFCell.ENCODING_UTF_16);
							if(rowList.get(n) != null){
					...
					HSSFSheet sheet = workBook.createSheet("Page " + i);//使用workbook对象创建sheet对象
					HSSFRow headRow = sheet.createRow((short) 0); //创建行，0表示第一行（本例是excel的标题）
					for (int j = 0; j < fieldName4.size(); j++) {//循环excel的标题
						HSSFCell cell = headRow.createCell(j);//使用行对象创建列对象，0表示第1列
						/**************对标题添加样式begin********************/
						//设置列的宽度
						sheet.setColumnWidth((short)j, (short)6000);
						...
						if(fieldName1.get(j) != null){
							//将创建好的样式放置到对应的单元格中
							cell.setCellStyle(cellStyle);
							cell.setCellValue((String) fieldName4.get(j));//为标题中的单元格设置值
						}else{
							cell.setCellValue("-");
						}
					}
					//分页处理excel的数据，遍历所有的结果
					for (int k = 0; k < (rows2 < SPLIT_COUNT ? rows2 : SPLIT_COUNT); k++) {
						if (((i - sheetNum1 - sheetNum2 - sheetNum3 -1) * SPLIT_COUNT + k) >= rows4)//如果数据超出总的记录数的时候，就退出循环
							break;
						HSSFRow row = sheet.createRow((short) (k + 1));//创建1行
						//分页处理，获取每页的结果集，并将数据内容放入excel单元格
						ArrayList<String> rowList = (ArrayList<String>) fieldData4.get((i - sheetNum1 - sheetNum2 - sheetNum3 -1) * SPLIT_COUNT + k);
						for (int n = 0; n < rowList.size(); n++) {//遍历某一行的结果
							HSSFCell cell = row.createCell(n);//使用行创建列对象
							cell.setCellValue(HSSFCell.ENCODING_UTF_16);
							if(rowList.get(n) != null){
